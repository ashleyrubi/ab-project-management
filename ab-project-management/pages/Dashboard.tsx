                  ...
                  <Link key={t.id} to={`/clients/${t.clientId}?taskId=${t.id}`} className="flex items-start gap-4 group">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[9px] font-bold text-gray-300 uppercase tracking-widest mt-1">
                        {getClientName(t.clientId)} • {format(new Date(t.updatedAt.seconds * 1000), 'HH:mm')}
                      </p>
                    </div>
                  </Link>
                ))
        ...
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-dark bg-opacity-40 backdrop-blur-sm">
          <div className="bg-white rounded-3xl shadow-2xl max-w-lg w-full p-0 overflow-hidden flex flex-col">
            ...
            <form onSubmit={handleLogTime} className="p-8 space-y-6 flex-grow overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-bold text-dark uppercase tracking-widest mb-1">Duration (minutes)</label>
                  <input type="number" className="w-full px-4 py-3 border border-gray-100 rounded-xl bg-canvas font-medium h-[48px]" value={logTimeForm.duration} onChange={e => setLogTimeForm({...logTimeForm, duration: e.target.value})} />
                </div>
                <div>
        ...
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-dark bg-opacity-40 backdrop-blur-sm">
          <div className="bg-white rounded-3xl shadow-2xl max-w-lg w-full p-0 overflow-hidden flex flex-col">
            ...
            <form onSubmit={handleSaveIdea} className="p-8 space-y-6 flex-grow overflow-y-auto">
              <div>
                <label className="block text-[10px] font-bold text-dark uppercase tracking-widest mb-1">Title</label>
                <input required autoFocus type="text" className="w-full px-4 py-3 border border-gray-100 rounded-xl bg-canvas font-medium h-[48px]" placeholder="Idea name..." value={saveIdeaForm.title} onChange={e => setSaveIdeaForm({...saveIdeaForm, title: e.target.value})} />
              </div>
              <div>
